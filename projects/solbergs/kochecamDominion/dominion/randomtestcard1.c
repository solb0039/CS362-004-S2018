...
int main() {
	...
	for (i =0; i < NUM_TEST; i++)
	{
		...
		for(curPlayer = 0; curPlayer < numPlayers; curPlayer++)
		{
			...
			if(( handCountBefore!=testG.handCount[curPlayer])||(deckCountBefore==testG.deckCount[curPlayer])||(playedCardCountBefore==testG.playedCardCount)||(numActionsBefore+2 != testG.numActions))
			{
				if(deckCountBefore != 0){
				printf("Failed test\n");
				failedTests++;}
				//Extra error reporting
				if (numActionsBefore+2 != testG.numActions)
